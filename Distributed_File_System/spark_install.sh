...
SPARK_TARBALL_URL="https://archive.apache.org/dist/spark/spark-2.4.4/spark-2.4.4-bin-hadoop2.7.tgz"
	...
	if $command >> $OUT_FILE; then
		...
		if check_dir "$SPARK_HOME_DIR"; then 
			...
			if $command >> $OUT_FILE; then
				...
				chown -R $USER_NAME:$USER_NAME $SPARK_HOME_DIR
			else
...
if [ $RUN_SPARK_DAEMONS -eq 1 ]; then 
	...
	command="su - $USER_NAME $SPARK_HOME_DIR/sbin/start-all.sh"
	if $command >> $OUT_FILE; then
