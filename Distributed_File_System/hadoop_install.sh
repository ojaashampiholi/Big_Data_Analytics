...
if [ $RUN_HADOOP_DAEMONS -eq 1 ] && [ $PASSWORDLESS_SSH_FLAG -eq 1 ]; then 
	...
	su -c  "$HADOOP_HOME_DIR/bin/hdfs namenode -format  > ~/namenode_format.out" - $USER_NAME
	if [ $? -eq 0 ]; then
		out "INFO - Formatting namenode complete."
	else
		out "ERROR - Error formatting namenode."
	fi
	out "INFO - Starting Hadoop daemons."
	su - $USER_NAME $HADOOP_HOME_DIR/sbin/start-dfs.sh
	if [ $? -eq 0 ]; then
		out "INFO - Hadoop daemons started."
	else
		out "ERROR - Error starting Hadoop daemons."
	fi
fi
